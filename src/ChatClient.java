...
public class ChatClient {
	...
	JTextArea constantUpdateArea = new JTextArea(20, 40);
	...
	public ChatClient() {
		...
		constantUpdateArea.setEditable(false);
		constantUpdateArea.setFont(f);
		frame.getContentPane().add(textField, "North");
		frame.getContentPane().add(messageArea, "Center");
		frame.getContentPane().add(new JScrollPane(constantUpdateArea), "East");
		frame.pack();
	...
	private void run() throws IOException {
		...
		while (true) {
			...
			if (line.startsWith("SUBMITNAME")) {
				...
				messageArea.setText(line.substring(7) + "\n" + messageArea.getText());
			} else if (line.startsWith("DATA")) {
				constantUpdateArea.setText(line.substring(4));
			}
		}
	}
